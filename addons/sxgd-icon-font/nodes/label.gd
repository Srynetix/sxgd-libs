...
const IconFont := preload("res://addons/sxgd-icon-font/nodes/font.gd")
...
func _update_font() -> void:
	var fa_font := IconFont.create_fa_font(IconFont.Family.Solid)
	add_theme_font_override("font", fa_font)
	add_theme_font_size_override("font_size", icon_size)
	var code := IconFont.get_icon_code(icon_name)
	if code != -1:
		text = char(code)

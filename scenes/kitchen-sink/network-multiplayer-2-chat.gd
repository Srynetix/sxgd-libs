...
class ChatServer:
	extends SxNetworkNodes.ServerHandler

	signal users_updated(users: Dictionary)

	var _logger: SxCore.LoggerExt.Logger
	var _users: Dictionary = {}  # int -> String

	func _init(peer_: SxNetworkNodes.ServerPeer) -> void:
		super._init(peer_)
		name = "ChatServer"
		_logger = SxCore.LoggerExt.get_logger("ChatServer")
...
class ChatClient:
	extends SxNetworkNodes.ClientHandler
	...
	func _init(peer_: SxNetworkNodes.ClientPeer) -> void:
		super._init(peer_)

		name = "ChatClient"
		_logger = SxCore.LoggerExt.get_logger("ChatClient")
...
@onready var _connect_panel := %ConnectPanel as MarginContainer
@onready var _bottom_panels := %BottomPanels as HBoxContainer

@onready var _server_panel := %ServerPanel as MarginContainer
...
@onready var _client_panel := %ClientPanel as MarginContainer
